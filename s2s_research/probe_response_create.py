...
logger = logging.getLogger("ProbeResponseCreate")

GIZMO_ID = "g-6a8f59bfdfc8819191e85d3cfa8fd722"

class StableDummyTrack(AudioStreamTrack):
    kind = "audio"
    def __init__(self):
        super().__init__()
        self._pts = 0
...
async def test_response_create():
    ...
    @dc.on("open")
    def on_open():
        logger.info("DC Open! Sending response.create with opening instructions...")
        # Direct instruction to speak the greeting
        event = {
            "type": "response.create",
            "response": {
                "instructions": (
                    "You are Aziz from OXzoon IPTV. The call is now connected. "
                    "Speak this exact opening greeting in French: "
                    "Bonjour ! Ici Aziz, responsable commercial chez OXzoon. Je vous appelle au sujet de nos abonnements IPTV. Est-ce un bon moment pour échanger ?"
                )
            }
        }
    ...
    logger.info("=== FULL MODEL SPOKEN GREETING ===\n%s\n", full)
    await pc.close()

if __name__ == "__main__":
    asyncio.run(test_response_create())
